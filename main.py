...
def display_campaign_details(campaign_uid: str):
    ...
    with col3:
        ...
        **Conversions:** {campaign['conversions']:,}  
        ...
        # Check if 'ctr' exists in campaign data
        if 'ctr' in campaign:
            st.markdown(f"**CTR:** {campaign['ctr']:.2f}%")
        else:
            # Calculate CTR on the fly
            calculated_ctr = safe_divide(campaign['clicks'] * 100, campaign['impressions'])
            st.markdown(f"**CTR:** {calculated_ctr:.2f}%")
    ...
    with tab1:
        ...
        with col1:
            ...
            # Use stored CTR if available, otherwise calculate
            if 'ctr' in campaign:
                ctr_value = campaign['ctr']
            else:
                ctr_value = safe_divide(campaign['clicks'] * 100, campaign['impressions'])
                
            funnel_text = [
                f"Impressions: {campaign['impressions']:,}",
                f"Clicks: {campaign['clicks']:,} (CTR: {ctr_value:.2f}%)",
        ...
        if ad_spend > 0 and clicks > 0 and conversions > 0 and revenue > 0:
            ...
            with col1:
                st.metric("Avg. CPC", f"${avg_cpc:.2f}", 
                        ...
                        delta_color="inverse")
                st.metric("CTR", f"{ctr:.2f}%",
                        delta=f"{ctr - campaign.get('ctr', ctr):.2f}%" if 'ctr' in campaign else None)
        ...
        for message in st.session_state.chat_messages:
            if message["role"] == "user":
                ...
                <div style="display: flex; justify-content: flex-start; margin-bottom: 10px;">
                    <div style="background-color: #e2e8f0; padding: 10px 15px; 
...
def display_amazon_listing_assistant():
    """Display a specialized AI chat interface for Amazon listing optimization."""
    st.title("Amazon Listing Optimization Assistant")
    
    st.markdown("""
    This specialized AI assistant helps you optimize your Amazon product listings for better visibility, 
    conversion rates, and overall performance. Get expert advice on titles, bullets, descriptions, keywords, 
    images, and A+ content specifically tailored for medical device products on Amazon.
    """)
    ...
    # Use custom container with Amazon styling
    st.markdown("""
    <div class="amazon-chat-header">
        <span class="logo">📦</span> Amazon Listing Optimization Assistant
    </div>
    """, unsafe_allow_html=True)
    ...
    # Input for new messages
    with st.form(key="amazon_chat_form", clear_on_submit=True):
        col1, col2 = st.columns([5, 1])
        
        with col1:
            user_input = st.text_area("Your message:", key="amazon_chat_input", height=100)
    ...
    topics = ["Product Title Optimization", "Bullet Points", "Backend Keywords", "A+ Content", "Image Best Practices", "Competitor Analysis"]
    
    # Create topic buttons in two rows
    col1, col2, col3 = st.columns(3)
    cols = [col1, col2, col3]
    ...
    for i, topic in enumerate(topics):
        with cols[i % 3]:
            if st.button(topic, key=f"topic_{i}"):
                # Add topic as a user message
                st.session_state.amazon_chat_messages.append({"role": "user", "content": f"Give me tips on Amazon {topic}"})
                st.rerun()
    
    # Clear chat button
    if st.button("Clear Chat History", key="clear_amazon_chat"):
        st.session_state.amazon_chat_messages = []
        st.rerun()
    ...
    if submit_button and user_input:
        # Add user message to chat history
        st.session_state.amazon_chat_messages.append({"role": "user", "content": user_input})
        
        # Get AI response
        with st.spinner("Generating response..."):
            # Check if API key is valid
            if st.session_state.api_key_status == "valid":
                ai_response = get_amazon_listing_assistant_response(st.session_state.amazon_chat_messages)
            else:
                ai_response = "API key is not configured or invalid. Please check your Streamlit secrets to ensure your OpenAI API key is correctly set."
        
        # Add AI response to chat history
        st.session_state.amazon_chat_messages.append({"role": "assistant", "content": ai_response})
    ...
    # Display chat messages
    with chat_container:
        for message in st.session_state.amazon_chat_messages:
            if message["role"] == "user":
                st.markdown(f"""
                <div style="display: flex; justify-content: flex-end; margin-bottom: 10px;">
                    <div style="background-color: #232f3e; color: white; padding: 10px 15px; 
                    border-radius: 15px 15px 0 15px; max-width: 80%; margin-left: auto;">
                        {message["content"]}
                    </div>
                </div>
                ...
                st.markdown(f"""
                <div style="display: flex; justify-content: flex-start; margin-bottom: 10px;">
                    <div style="background-color: #fcfcfc; padding: 10px 15px; border: 1px solid #e2e8f0;
                    border-radius: 15px 15px 15px 0; max-width: 80%;">
                        {message["content"]}
                    </div>
                </div>
    ...
    # Sample questions if chat is empty
    if not st.session_state.amazon_chat_messages:
        st.markdown("### Try asking about:")
        
        st.markdown("""
        - How do I optimize my product title for a medical device?
        - What are the best practices for bullet points in healthcare products?
        - How can I use backend keywords effectively for my mobility aids?
        - What images should I include for my medical product listing?
        - How do I create effective A+ content for medical devices?
        - What pricing strategies work best for healthcare products on Amazon?
        - How can I improve my BSR (Best Seller Rank) for medical devices?
        - What should I include in my product description for a TENS unit?
        - How do I set up variations for different sizes of medical braces?
        - What are common mistakes to avoid in medical device listings?
        """)
        
        # Add a sample medical device listing example
        with st.expander("Sample Medical Device Listing Analysis", expanded=False):
            st.markdown("""
            ### Sample Listing Analysis: Mobility Scooter
            
            #### Current Title
            "Vive Mobility Scooter - Electric Powered Wheelchair Device - Motorized Power Scooters for Seniors - 4 Wheel - Folding Mobility Chair - Long Range Battery - Adjustable Seat and Arms"
            
            #### Analysis
            - **Title is too long** (96 characters). Amazon recommends 80-150 characters, but front-loading key terms is crucial.
            - **Keyword stuffing** is apparent with repeated terms like "scooter," "mobility," and "wheelchair."
            - **Missing key specifications** like weight capacity, speed, or battery range in miles/hours.
            
            #### Recommended Title
            "Vive Mobility Scooter for Seniors (300lb Capacity) - Foldable 4-Wheel Electric Wheelchair with 25-Mile Battery Range - Adjustable Seat & Arms"
            
            #### Benefits of Optimization
            - Front-loads the most important search terms
            - Includes specific, valuable details (weight capacity, range)
            - More readable format that appeals to both the algorithm and human shoppers
            - Likely to improve click-through rates from search results
            """)
...
def display_settings_page():
    ...
    with tab3:
        ...
        **API Features:**
        ...
        - Get Amazon listing optimization advice in the Amazon Listing Assistant
...
def setup_sidebar():
    ...
    with st.sidebar:
        ...
        nav_options = [
            ...
            ("Amazon Listing Assistant", "shopping-cart", "amazon_assistant"),
...
def main():
    ...
    elif st.session_state.view == "amazon_assistant":
        display_header()
        display_amazon_listing_assistant()
