...
def load_custom_css():
    ...
    <style>
        ...
        /* Improvement over regular sliders */
...
def display_tooltip(label, help_text):
...
class ReturnOptimizer:
    ...
    def add_scenario(self, scenario_name, sku, sales_30, avg_sale_price, sales_channel,
                     ...
                     tag=None, notes=None, confidence_level=None, risk_rating=None,
                     implementation_time=None, implementation_effort=None):
        ...
        try:
            ...
            monthly_net_benefit = (annual_savings - annual_additional_costs) / 12
            
            # Calculate ROI metrics
            roi = break_even_days = break_even_months = score = None
            if solution_cost > 0 and monthly_net_benefit > 0:
                roi = ((annual_savings - annual_additional_costs) / solution_cost) * 100  # as percentage
    ...
    def run_monte_carlo_simulation(self, scenario_uid, num_simulations=1000, param_variations=None):
        """
        ...
        for i in range(num_simulations):
            ...
            sim_monthly_net_benefit = (sim_annual_savings - sim_annual_additional_costs) / 12
            ...
            if sim_solution_cost > 0 and sim_monthly_net_benefit > 0:
                sim_roi = ((sim_annual_savings - sim_annual_additional_costs) / sim_solution_cost) * 100
                sim_break_even_months = sim_solution_cost / sim_monthly_net_benefit
    ...
    def get_aggregate_statistics(self):
        ...
        stats = {
            ...
            'avg_roi': self.scenarios['roi'].mean(),
            'median_roi': self.scenarios['roi'].median(),
            'avg_break_even': self.scenarios['break_even_months'].mean(),
        ...
        if not self.scenarios[self.scenarios['roi'] > 0].empty:
            best_idx = self.scenarios[self.scenarios['roi'] > 0]['roi'].idxmax()
            stats['best_scenario'] = {
        ...
        if not self.scenarios[self.scenarios['roi'] > 0].empty:
            worst_idx = self.scenarios[self.scenarios['roi'] > 0]['roi'].idxmin()
        ...
        if stats['total_investment'] > 0:
            stats['portfolio_roi'] = ((stats['total_annual_savings'] - stats['total_additional_costs']) / 
...
def create_scenario_form(wizard_mode=False):
    ...
    with st.form(key="scenario_form"):
        ...
        # Display steps if in advanced mode
        if st.session_state.app_mode == "Advanced":
        ...
        with col1:
            scenario_name = st.text_input("Scenario Name", help="A memorable name for this return reduction strategy")
        ...
        with col2:
            ...
            if st.session_state.app_mode == "Advanced":
                notes = st.text_area("Notes", help="Additional details about the solution")
            else:
                notes = None
        ...
        if st.session_state.app_mode == "Advanced":
            ...
            implementation_effort = st.slider("Implementation Effort (1-10)", 1, 10, 5,
    ...
    # Step 4: Cost and Impact
        ...
        if sales_30 > 0 and returns_30 > 0:
            ...
            with col2:
                ...
                st.markdown(f"**New Return Rate:** {return_rate * (1 - reduction_rate/100):.2f}%")
        
        # Allow final edits
        st.markdown("### Final Adjustments (Optional)")
        st.caption("Make any final adjustments before saving")
...
def display_scenario_financials(scenario):
...
def display_scenario_returns(scenario):
...
def display_scenario_risk(scenario):
...
def display_portfolio_analysis(df):
    ...
    if st.session_state.app_mode == "Advanced":
        ...
        avg_roi = df['roi'].mean()
        avg_breakeven = df['break_even_months'].mean()
...
# Display help modal
def display_help():
    """Display help information in a Streamlit modal"""
